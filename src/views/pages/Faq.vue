...
<template>
    <div id="faq-page ">
        <!-- JUMBOTRON -->
        <div class="vx-row pl-5 pr-5 ml-5 mr-5">
            <div class="vx-col w-full md:w-2/4 lg:w-1/4 rounded-lg">
                <vx-card>
                    <h4 class="pb-5">Table des matières</h4>
                    <hr class="mb-5">
                    <ul class="faq-topics w-full" v-for="lesson in lessons">
                        <li v-for="item in lesson.programme" :key="item.id" class="p-2 font-medium flex items-center" @click="faqFilter = item.id">
                          <vs-button style="width:1px!important; height:1px"class=" mr-5" radius :color="item.couleur" type="filled" ></vs-button> <span  class="cursor-pointer text-black font-bold">Chapter {{item.id}}: {{ item.titre }}</span>
                        </li>
                    </ul>
            ...
            <!-- FAQ COL -->
            <vx-card  class="vx-col  w-full md:w-3/5 lg:w-3/4 mt-12 md:mt-0 bg-white pr-0 pl-0">
              <div v-for="lesson in lessons" class="flex flex-row mt-5 pt-5">
                <vx-card card-background="primary" class="m-2 pb-3 text-white" v-for="item in lesson.exo" v-if="item.id <= 4">
                  <router-link  :to="{ name: 'page-search', params: {id: item.id} }"  >
                    <ul  class="pt-0 mt-0 " >
                      <vs-button color="dark" type="line" class=" w-full h-full mt-4 mb-4 pt-4 pb-4">
                        <li class="text-center ">
                          <h5 class="text-white text-base">{{item.titre}}</h5>
                        </li>
                      </vs-button>
                    </ul>
                  </router-link>
                </vx-card>
                <vs-card  class=" hidden" v-for="item in lesson.exo" v-else>
                  <ul  class="pt-0 mt-0" >
                    <vs-button type="line">
                      <li class="text-center">
                        <h5>{{item.titre}}</h5>
              ...
              <div v-for="lesson in lessons" class="flex flex-row mt-5 pt-5">
                <vx-card  class="m-2 pb-3" card-background="grey" v-for="item in lesson.exo" v-if="item.id > 4 && item.id <=8">
                  <router-link  :to="{ name: 'page-search', params: {id: item.id} }"  >
                    <ul  class="m-0 p-0" >
                      <vs-button color="dark" type="line" class=" w-full h-full mt-4 mb-4 pt-4 pb-4" >
                        <li class="text-center">
              ...
              <div v-for="lesson in lessons" class="flex flex-row pt-5 mt-5">

                <vx-card card-background="primary" class="m-2 pb-3"  v-for="item in lesson.exo" v-if="item.id > 8">
                  <router-link  :to="{ name: 'page-search', params: {id: item.id} }"  >
                  <ul  class="pt-0 mt-0 " >
                    <vs-button color="dark" type="line" class=" w-full h-full mt-4 mb-4 pt-4 pb-4" >
                      <li class="text-center">
                        <h5 class="text-white">{{item.titre}}</h5>
                      </li>
                    </vs-button>
                  </ul>
                  </router-link>
...
export default{
    ...
    mounted () {
      axios
        .get('http://localhost:3000/lessons',{params:{id: this.$route.params.id}})
